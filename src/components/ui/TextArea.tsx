...
const Input = forwardRef<HTMLInputElement, InputProps>((_props, ref) => {
  ...
  return (
    <div className="relative mb-6 flex flex-col align-top">
      <label className="pointer-events-auto absolute -top-2.5 left-4 z-10 block select-none truncate whitespace-nowrap bg-white p-[0px_5px] text-[12px] font-normal tracking-[0.2px]">
        {label}
        {required && ' *'}
      </label>
      <input
        ref={ref}
        className="block h-14 rounded border border-solid border-ash-100 px-3.5 py-4 font-heading font-normal leading-6 outline-0"
        {...props}
      />
    </div>
  )
